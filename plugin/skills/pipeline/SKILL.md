...
**If `policy_warnings` is non-empty, print each one and keep going.** None are emitted today; every
shipped policy, single-model included, uses the 1h subagent cache (see the cache-TTL check).
