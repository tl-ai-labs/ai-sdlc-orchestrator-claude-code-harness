...
```
A brownfield run is in progress (run_id: <run_id>, phase: <phase>). Finish it or /sdlc:revert <run-id> before changing the policy.
```
...
- The browser opens only when the user picks `Author a new policy` under `change` (or the same
  option during first-time `/sdlc:setup`). Picking a shipped preset stays in the terminal.
