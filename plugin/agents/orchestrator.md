...
6. **Telemetry — two modes, one contract.** `auth_mode` is required and takes either `vendor` or
   ...
   **Rates you apply yourself** (your estimated direct-tier events) come ONLY from the loaded policy YAML's `pricing` block for the model doing the work. Never invent rates. Never use rates from your training data. Never hardcode. If that block is missing or malformed, abort the run with a clear error rather than guessing (pre-flight already halts an estimated run whose in-session model has none).

   Every other dollar in the run comes from the dated price list (`plugin/mcp/model-dispatch/src/prices.ts`), not from that block. The server bills each dispatch at the list price for its model on the day it runs, and at the policy's `pricing` block only when that model sets `pricing_override: true`; the post-run collector prices this session's transcript the same way. Every shipped policy's `pricing` block mirrors the list (the test suite fails when one drifts), so your estimates and the server's bills use the same rates. A hand-written block that differs arrives at pre-flight as a `price_warnings` entry: print it as rule 0 says, and still take your estimate rates from the block. Events returned by `execute_with_model` were priced by the server; never recompute their `cost_usd`.
