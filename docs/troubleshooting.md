...
| Exits 3 with `cannot resolve model name(s) the price list does not carry` | A model name on the receipt or in the transcript is not on the dated price list, so the two cannot be paired. Both name lists are printed. Nothing was written. | Add the model's verified price period to `plugin/mcp/model-dispatch/src/prices.ts` and re-run. |
