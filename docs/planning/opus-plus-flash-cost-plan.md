...
| Opus-only got cheaper from the same clean-up | Runs 15 → 16: $20.10 → $14.86 once the scout was skipped outright, the plan kept brief and a parser re-delegation removed (0.7.9) | A single-model user benefits too; the comparison target moved down with the arm we were trying to beat |
...
| Run-to-run noise exceeds the gap | Opus-only on the same brief: $14.86, $17.40, $17.96, $20.10, $22.91 | A single pair cannot settle a difference under ≈ $2; Row 5 (shared prefix) and repeated pairs are the next measurement step, not more code |
