...
| 6 | **Deterministic packet planning**: `plan-to-packets.mjs` emits `packets.json` from the linted plan's unit sections (File / Action / Depends / Mirror / Verify are already structured) — zero model calls; Opus only reviews the list | dispatching policies | next | 19.16 | expected ≈ 16.4 | −$1.0: `plan_task_packets` was $1.03 (9 Opus turns) on the row-4 run |
| 7 | **Flash repo scout before the architect**: one mechanical packet per requirement area returns the mirror / anchor candidates (`path:lines`) from the discovery file list; the architect reads those slices instead of hunting the repo | dispatching policies | next | 19.16 | expected ≈ 14.5 | −$2: the row-4 architect took 110 messages / 486k cache writes / $3.75 finding mirrors, vs 61 messages / $2.11 in opus-only |
