...
| 9 | **Batch dispatch**: `execute_batch` runs every apply-form packet of the phase in one call — parallel (default 4) in `depends_on` order, one writer per file at a time, one receipt per packet | dispatching policies | **implemented 2026-09-21 on `feat/batch-dispatch` (0.8.0), not yet measured** | 14.86 (run 16, opus-only lean path, still at 1h) | expected ≈ 15.5 | −$1.5: execute_packets was 25 turns / $3.21 for 21 packets on run 14 |
