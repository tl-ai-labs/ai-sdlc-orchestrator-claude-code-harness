...
| 4 | **Architect writes a spec, not the file** — `plan-lint.mjs` gate (≤ 12 lines per fenced block, ≤ 150 fenced lines, no "Content:" bodies), per-unit Exports / Behavior / Mirror / Edit-anchor contract, one `House style` section, 3-input worker packets | dispatching policies (opus-only plans already pass the lint: 0 fenced lines on both baselines) | **implemented 2026-09-18 on `feat/architect-spec-not-program`, not yet measured** | 19.16 (5m TTL control, no re-run needed) | expected ≈ 16–17 | −4 expected: plan 1,041 → ~450 lines removes ≈ 25k Opus output + what two reviewers re-read; other row-4 follow-ups (batch dispatch, report on Flash) still open |
