...
| Model names | Compared as written, so the receipt's `claude-opus-5[1m]` never matched the transcript's `claude-opus-5` | Read through `resolveModel`: an exact id, an id plus `-YYYYMMDD`, or either plus a bracketed Claude Code option such as `[1m]`. With a receipt, a name on either side that it cannot read stops the collector (exit 3); without one, that model's tokens are listed under `unpriced` and the figure is labelled `INCOMPLETE` (exit 1 with `--strict-pricing`). |
