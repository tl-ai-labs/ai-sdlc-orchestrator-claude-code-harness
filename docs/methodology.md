...
- **The receipt.** When the run kept Claude Code's own end-of-session result (`claude -p --output-format json`, or a runner's `claude-session.json` beside the manifest; `--receipt <file>`), its `modelUsage[model].costUSD` is what Anthropic's price table multiplied. The transcript is priced and compared. Below the receipt by more than 5% (`--receipt-tolerance`) the tool refuses with exit 3 and writes nothing: the receipt cannot over-report, so the transcript tree is missing billed messages (a subagent file not copied, a window that starts too late). At or above it, the transcript figure is written and the receipt is recorded beside it as the floor — a session file can hold a preamble the receipt never billed, and those messages are inside the run's own window. Both numbers are always printed.
...
- **Window anchor.** Transcripts carry no run id, so the window is bounded by time: it opens at the driver's own `run.start` line in `.sdlc/runs/<run-id>/orchestrator.log` (the last one at or before the manifest's `started_at`, so a reused run id never reaches back into an earlier run) and closes at the manifest's `ended_at`, ± 5 minutes, applied per-message via each line's own timestamp. The manifest's `started_at` is the first *dispatched* event, not the driver's start; opening the window there dropped every driver message before the first dispatch — 22% of the driver's spend on a real run, and a receipt check that failed for it. With no `run.start` line the window falls back to `started_at` and the collector's window line says so. File-level pruning uses an mtime **lower bound only** — an upper bound would silently drop the run's messages whenever the session kept going after the run ended.
