...
- **The receipt.** When the run kept Claude Code's own end-of-session result (`claude -p --output-format json`, the last `result` line of a `--output-format stream-json` capture, or a runner's `claude-session.json` beside the manifest; `--receipt <file>`), its `modelUsage` is the CLI's own per-model token counts and dollars for the one invocation it billed. The check is exact and per token bucket. For every model on the receipt, the window's input, cache-read and cache-write tokens must **equal** the receipt's, and its output tokens must not exceed them (an interrupted message has no terminal line, so its output never reaches the transcript). Equal everywhere means the window *is* that invocation: the receipt's dollars are booked, labelled `receipt (transcript agrees, ±x%)`, with the transcript-priced figure kept beside them. A bucket **below** the receipt means the transcript tree is missing billed messages (a subagent file not copied, a window that opens too late): exit 3, nothing written. A bucket **above** it means the window holds messages the receipt never billed. If the pinned session file shows a second human turn inside the window — a `--resume` continuation restarts the bill — the receipt is checked against the last invocation alone, and the whole window is written transcript-priced under a label that says only the last invocation was verified; with no such turn, exit 3. There is no tolerance: a 5% margin admits a whole missing message, and treating the receipt as a floor once booked a preamble's eight messages onto a real run (fixture `pass1`, +5.8%). A receipt for another session, and a rate card that no longer reproduces the receipt's own dollars, are each named on stderr; the first is refused, the second is booked at the receipt and noted. Both numbers are always printed.
