...
| Who types a code file in the two-model policies (every run, with or without `--executor`) | A code packet went to the cheaper model only when its `task_type` was on a list written for NestJS/React projects (`controller_handler`, `dto`, `react_component`, ...); any other type, and the catch-all `other`, fell to the default rule — Opus — so on a project in another language most files were typed by Opus | By stage alone, whatever the file's language, name or type: the codegen rule of `opus-plus-flash`, `-v37`, `-v38`, `opus-plus-sonnet` and `-max` matches on phase only (`test/stageRouting.test.mjs`). The executor sends no task type, and refuses a policy whose rule for one of its stages also matches on `task_type`, `module` or `intent` — such a rule could only be missed, silently. A spec unit carries no file-type label |
| The executor's checks on a typed file | Parsed Python, TypeScript/JavaScript and JSON; every other language was checked for being non-empty only | The same for every language: the right path, a safe path, not empty. Whether a file is right, syntax included, is judged by the project's own build and tests and the repair rounds |
