 ...
 *
 * The rules live in CONTRIBUTING.md and CLAUDE.md.
 *
 ...
 *     Claude-instruction files where "the user" is the correct third-person
 *     reference.
 *   - node_modules/, dist/, .venv/, .claude/, .sdlc/, src/, examples/ are
...
test("no AI-slop terms in user-facing docs", () => {
  ...
  assert.deepEqual(
    offenders,
    [],
    "Slop terms in shipped docs. Rewrite in plain factual voice; see CLAUDE.md.",
  );
});
...
test("user-facing docs are written in the second person, not the third", () => {
  ...
  assert.deepEqual(
    offenders,
    [],
    'Third-person voice in user docs. Rewrite as "you do X"; see CLAUDE.md.',
  );
});
...
test("no AI-slop terms in source-file comments and docstrings", () => {
  ...
  assert.deepEqual(
    offenders,
    [],
    "Slop terms in source comments. Rewrite factually; see CLAUDE.md.",
  );
});
