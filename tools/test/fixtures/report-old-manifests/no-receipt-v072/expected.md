...
**Scope: dispatched work + orchestrator overhead** — the run's own loop ($16.1525, transcript-measured) is a separate line in Costs, never blended into dispatched totals
...
| Orchestrator overhead (transcript-measured) | $16.1525 |
| In-session dispatch, subtracted once | −$3.7394 |
| **True total (dispatched − in-session + orchestrator)** | **$16.1525** |

_UNVERIFIED — transcript (no receipt; unverified; LOWER BOUND — window opens at the first dispatch); no receipt was found beside the manifest, re-run the collector after a headless session exits, or keep its claude -p result._

_Window 2026-08-16T05:12:06.918Z → 2026-08-16T06:09:13.802Z (approximate: opens at manifest started_at - 5m, closes at manifest ended_at + 5m)._
